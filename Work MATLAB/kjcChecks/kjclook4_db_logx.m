function[]=kjclook4_db_logx(filename,pathname,refch)
...
% Try here to add background curve
    if bg=='NoGo'
    else
        hold on
        plot(bgf,bgcoh(ich,:),bgf(iw),bgcoh(ich,iw),'.');
    end
% End of try here to add background curve
