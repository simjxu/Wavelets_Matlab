function[]=kjclook2_db_logx(filename,pathname,refch,squared)
% Quicklook2 - computes coherence of data
...
[Nch,Nf]=size(A);  %Nch is number of channels, Nf is the number of spectral lines
%fs=3200*2.56;  % fs??
...
coh=abs(C(ch_num-refch,1:Nf))./(sqrt(A(ch_num,1:Nf)).*sqrt((ones(numch,1)*A(refch,1:Nf))));
if squared
    coh=coh.^2;
end
...
title(sprintf('Coherence of %s, Ref Num %g',filename,refch))
